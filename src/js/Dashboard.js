export default class Dashboard {
  ...
  setupEventListeners() {
    this.createInstanceButton.addEventListener("click", () => {
      fetch(`${this.serverUrl}/instances`, { method: "POST" });
    });
  }

  renderInstances() {
    this.instancesContainer.innerHTML = this.instances
      .map(
        (instance) => `
          <div class="instance">
            <div>${instance.id}</div>
  ...
  startInstance(id) {
    fetch(`${this.serverUrl}/instances/${id}/start`, { method: "PUT" });
  }

  stopInstance(id) {
    fetch(`${this.serverUrl}/instances/${id}/stop`, { method: "PUT" });
  }

  deleteInstance(id) {
    fetch(`${this.serverUrl}/instances/${id}`, { method: "DELETE" });
  }
}
